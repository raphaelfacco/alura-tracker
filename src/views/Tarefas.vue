    ...
    <Modal :mostrar="tarefaSelecionada != null">
      ...
      <template v-slot:corpo>
          <div class="field">
              ...
              <input type="text" class="input" v-model="tarefaSelecionada.descricao" id="descricaoDaTarefa" v-if="tarefaSelecionada"/>
          </div>
      </template>
  ...
  methods: {    
    salvarTarefa (tarefa:ITarefa) : void {      
      this.store.dispatch(CADASTRAR_TAREFA, tarefa)
        .then(() => this.lidarComSucesso("cadastrada"));
      
    },  
    lidarComSucesso(operacao: string){      
        this.notificar(
          TipoNotificacao.SUCESSO, 'Excelente!', `A tarefa foi ${operacao} com sucesso!`
        )
        this.$router.push('/')   
    },
